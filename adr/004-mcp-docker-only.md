...
- **Development friction** -- developers must rebuild the image after changes (`npx tsc && docker build -t rewelo-mcp .`). For local development, tests run outside Docker via `npm test` (Node's built-in test runner).
...
- Local development and testing continue to use `node` and its built-in test runner directly -- Docker is only required for the MCP server runtime.
