...
import { z } from "zod";
import { apiRoutes } from "../dashboard/api.js";
import { dashboardHtml, loginHtml } from "../dashboard/html.js";
...
export function createMcpServer(dbPath: string, options?: { maxRequestsPerSecond?: number }): McpServer {
  ...
  server.tool(
    "token_create",
    "Create a bearer token for HTTP API/MCP auth. Returns the raw token (shown once).",
    {
      label: z.string().describe("Human-readable name for the token"),
      readonly: z.boolean().optional().describe("Read-only access (default false)"),
      project_names: z.array(z.string()).optional().describe("Restrict to these projects (omit for all)"),
      expires_in_days: z.number().optional().describe("Expire after N days (omit for no expiry)"),
    },
    async ({ label, readonly, project_names, expires_in_days }) => {
      try {
        return await withDb(async (db) => {
          ...
          const raw = await createToken(db, label, {
            readonly,
            projectIds,
            expiresInDays: expires_in_days,
          });
          return textResult({ token: raw, label, readonly: readonly ?? false, projects: project_names ?? "all" });
        });
      } catch (err) {
        return errorResult(err);
      }
  ...
  server.tool(
    "token_list",
    "List all auth tokens (without revealing token values).",
    {},
    async () => {
      try {
        const tokens = await withDb((db) => listTokens(db));
  ...
  server.tool(
    "token_revoke",
    "Revoke an auth token by label.",
    {
      label: z.string().describe("Label of the token to revoke"),
    },
    async ({ label }) => {
      try {
        const ok = await withDb((db) => revokeToken(db, label));
        if (!ok) throw new Error(`Token "${label}" not found`);
        return textResult({ revoked: label });
      } catch (err) {
        return errorResult(err);
      }
    }
...
/**
 * Start a unified HTTP server that handles:
 *   - MCP Streamable HTTP on /mcp (POST + GET for SSE)
 *   - Dashboard API on /api/*
 *   - Dashboard UI on /
 */
export async function startHttpServer(dbPath: string, port: number): Promise<void> {
  const validDbPath = validateDbPath(dbPath);

  // Open shared DB for the dashboard API routes
  const dashDb = await DB.open(validDbPath);
  await migrate(dashDb);

  // Map of session ID -> transport (for stateful MCP sessions)
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    // ---- Login endpoint (sets cookie, no auth required) ----
    if (url.pathname === "/login" && req.method === "POST") {
      ...
      req.on("end", async () => {
        try {
          ...
          const { verifyToken } = await import("../auth/repository.js");
          const scope = await verifyToken(dashDb, token);
          if (!scope) {
          ...
          res.writeHead(200, {
            ...
            "Set-Cookie": `rw_token=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=86400`,
          });
    ...
    if (url.pathname === "/logout" && req.method === "POST") {
      res.writeHead(200, {
        "Content-Type": "application/json",
        "Set-Cookie": "rw_token=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0",
      });
      res.end(JSON.stringify({ ok: true }));
      return;
    }
    ...
    if (url.pathname === "/mcp" || url.pathname.startsWith("/api/") || url.pathname === "/") {
      const auth = await authenticate(dashDb, req, res);
      if (!auth.authenticated) return;

      // For the dashboard root, if auth is enabled but no token provided,
      // show login page instead of 401
      if (url.pathname === "/" && !auth.scope) {
        const { hasTokens } = await import("../auth/repository.js");
        if (await hasTokens(dashDb)) {
      ...
      // Store scope on request for downstream use
      (req as any)._authScope = auth.scope;
    }
    ...
    if (url.pathname === "/mcp") {
      ...
      if (req.method === "POST") {
        // For new sessions (no session ID or initialization), create transport + server
        if (!sessionId || !transports.has(sessionId)) {
          ...
          const mcpServer = createMcpServer(dbPath);
      ...
      if (req.method === "GET") {
        ...
        if (sessionId && transports.has(sessionId)) {
          ...
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Missing or invalid session ID for SSE" }));
        }
        return;
      }
    ...
    if (url.pathname.startsWith("/api/")) {
      try {
        const result = await apiRoutes(dashDb, url);
  ...
  httpServer.listen(port, () => {
    console.log(`rewelo server running at http://localhost:${port}`);
